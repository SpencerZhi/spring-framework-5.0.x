...
```
	...
	1.mybatis -- sqlSession -- defaultSqlSession -- defaultSqlSession.select -- sql
	2.spring-mybatis -- sqlSession -- sqlSessionTemplate -- sqlSessionTemplate.select -- proxy.invoke -- select -- sql
...
```
SpringBean实例化之前： @MapperScan主要做了以下3件事
	1.通过mapperScan扫描，扫描出所有的mapper所对应的BeanDefifition. 放入bdMap当中。
	2.把mapper变成 mapperFactoryBean, MapperFactoryBeanDefinition
...
SpringBean实例化之中和之后：
	mybatis使用 spring的初始方法扩展点来完成 mapper信息的初始化，如sql语句初始化。
...
```
首先了解知识点：
	1.自动装配用 byType(set(xxx)) ，mybatis这里是 byType
...
为什么 mybatis中的 configuration会包含 statementId?
	...
	里面的 MappedStatement包含所有 mapper信息（statement）。 
	什么时候放入的？spring初始化时，会读取 mapper所有信息，放到这个 mappedStatements中
	statement  由包名+类名+方法名组成
	
mybatis在 spring中自动装配的属性有哪些?  MapperFactoryBean
	mapperInterface	---class
	addToConfig	---已经给了，被包含
	setSqlSessionFactory 1
	setSqlSessionTemplate 1
